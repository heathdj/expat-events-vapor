...
- **Destructive actions confirm before firing.** Any control that cancels, removes, revokes, or otherwise undoes something a person already has (cancelling an event, leaving a group/event, removing a moderator's role, and — in later milestones — things like account deletion) gets `hx-confirm="..."` on its `<form>`, htmx's own confirm mechanism. Use `hx-confirm` alone, not also a matching `onsubmit="return confirm(...)"` on the same form — both together shows the dialog twice in a row, which is itself a bug this convention exists partly to prevent (found in M4, fixed when this convention was written down in M6). Purely additive actions (join, promote, create) don't need one.
