...
## M5 — Realtime chat — 🚧 in progress

`ChatMessage` model and `ChatMessageDTO`/`ChatEnvelope` wire types exist (M1/shared package); building the `ChatRoomRegistry` actor, WebSocket route, and htmx `ws` client wiring now (architecture §8, plan §M5).

- **Carried forward from M2**: the async/await test-warning fix (`WARNINGS.md` row 4 — all 7 `app.test(...)` call sites marking their trailing closure `async`) was committed locally on `milestone/m2-auth` but never pushed before PR #4 was merged, so it's not in `main`. Re-applied here via cherry-pick as this branch's first commit rather than lost or requiring reopening an already-merged PR.
- **Design system, incremental application starts here**: `docs/design-system.md` (extracted from the design canvas) and the extracted image assets (`Server/Public/design/`) are committed as of this branch — first real use of them will be in this milestone's own screens (event chat UI) rather than a retroactive pass over M1/M2/M4's existing pages, per the earlier decision to apply styling incrementally per milestone rather than as its own dedicated milestone.
