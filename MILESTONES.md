...
## M5 — Realtime chat — 🚧 code complete, build/test not yet run, human server-checkpoint pending
...
1. Two concurrent sessions both seeing a message live, without reload — implemented (`ChatRoomRegistry.broadcast` fans a rendered HTML fragment out to every open socket on the event, including the sender, over `hx-swap-oob`) but **not yet verifiable by an automated test** — XCTVapor has no WebSocket test client wired into this suite. This is the one criterion that needs a real human server-checkpoint (two browser tabs, same event) before M5 can be called done, the same way M2's OAuth click-through needed one.
...
6. Reload after disconnect still shows full history — `ChatWebController.handle` replays full history over a freshly-opened socket in addition to what the page already rendered on load, so a client that reconnects without a full page reload still gets the complete thread. ✅ (covered indirectly by the same history tests as #2; the reconnect-specific path is exercised for real at the item-1 checkpoint.)
